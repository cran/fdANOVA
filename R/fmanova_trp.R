fmanova.trp = function(x, group.label, k = 30, projection = c("GAUSS", "BM"),
  ...
  if(! is.logical(permutation)){ stop("argument permutation is not logical") }
  if(B < 1){ stop("invalid number of permutations (B)") }
  if(any(k < 1)){ stop("invalid number of projections (k)") }
  if(is.null(k)){ stop("argument k is not a vector of length greater than or equal to one") }
  ...
  manova.statistics.quick = function(data, group.label, n, p){
    ...
    for(ii in 1:l) xmeans[, ii] = matrix(colMeans(data[group.label == group.label0[ii], ]), nrow = p, ncol = 1)
    H = matrix(0, nrow = p, ncol = p)
    for(iiH in 1:l) H = H + n.i[iiH] * (xmeans[, iiH] - gmeans) %*% t(xmeans[, iiH] - gmeans)
    E = matrix(0, nrow = p, ncol = p)
    for(iiE in 1:l) E = E + (n.i[iiE]-1) * cov(data[group.label == group.label0[iiE],])
    wart = (eigen(H %*% solve(E)))$values
    return(c(Wilks = prod(1/(1+wart)), LH = sum(wart), Pillai = sum(wart/(1+wart)), Roy = wart[1]))
  }
  ...
  for(ik in k){
    if(projection == "GAUSS"){
      if(parallel.method == "parallel.method0"){
        ik.data.proj = list()
        W = numeric(ik); LH = numeric(ik); P = numeric(ik); R = numeric(ik)
        for(j in 1:ik){
          ...
          if(permutation == FALSE){
            W[j] = summary(model, test = "Wilks")$stats[1, 6]
            LH[j] = summary(model, test = "Hotelling-Lawley")$stats[1, 6]
            P[j] = summary(model, test = "Pillai")$stats[1, 6]
            R[j] = summary(model, test = "Roy")$stats[1, 6]
          }else{
            ...
            Wp = numeric(B); LHp = numeric(B); Pp = numeric(B); Rp = numeric(B)
            for(i.perm in 1:B){
              manovap = manova.statistics.quick(data.proj, sample(group.label), n, p)
              Wp[i.perm] = manovap[1]; LHp[i.perm] = manovap[2]; Pp[i.perm] = manovap[3]; Rp[i.perm] = manovap[4]
            }
            W[j] = mean(Re(Wp) <= Ws); LH[j] = mean(Re(LHp) >= LHs); P[j] = mean(Re(Pp) >= Ps); R[j] = mean(Re(Rp) >= Rs)
          }
        }
        iik = iik + 1
        all.data.proj[[iik]] = ik.data.proj
        pvalues[, iik] = c(min(ik*W[order(W)]/1:ik), min(ik*LH[order(LH)]/1:ik),
                           min(ik*P[order(P)]/1:ik), min(ik*R[order(R)]/1:ik))
      }
      if(parallel.method == "parallel.method1"){
        ...
        rs = foreach(j = 1:ik, .combine = "c") %dopar%
        {
          ...
          if(permutation == FALSE){
            ...
            list(c(mean(Re(Wp) <= Ws), mean(Re(LHp) >= LHs), mean(Re(Pp) >= Ps), mean(Re(Rp) >= Rs)),
                 data.proj)
          }
        }
        ...
        pvalues[, iik] = c(min(ik*(rs[, 1])[order(rs[, 1])]/1:ik), min(ik*(rs[, 2])[order(rs[, 2])]/1:ik),
                           min(ik*(rs[, 3])[order(rs[, 3])]/1:ik), min(ik*(rs[, 4])[order(rs[, 4])]/1:ik))
      }
    }else{
      if(parallel.method == "parallel.method0"){
        ik.data.proj = list()
        ...
        for(j in 1:ik){
          ...
          for(i in 1:p){
            data.matrix = as.matrix(t(x[[i]]))
            bm.p = cumsum(c(mean(x[[i]][1,]), rnorm(I-1, mean = 0, sd = 1)))/sqrt(I)
